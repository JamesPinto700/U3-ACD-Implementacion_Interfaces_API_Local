 ...
 */
public class CalculatorForPrimeNumbersCubedUpToTheLimit implements Succesionable {
    private Integer limit;
    private Integer currentPrime;
    private StringBuilder printableTerms;

    public CalculatorForPrimeNumbersCubedUpToTheLimit(Number limit) {
        this(0, limit);
    }

    public CalculatorForPrimeNumbersCubedUpToTheLimit(Number start, Number limit) {
        if(start.intValue() < 0){
    ...
    public Number calculate() {
        ...
        while(currentPrime < limit){
            this.printableTerms.append(currentPrime).append("^3").append(" + ");
            result += Math.pow(this.currentPrime, 3.0);
